...
const d={level:4,pbonus:2,scores:{Força:10,Destreza:15,Constituição:12,Inteligência:16,Sabedoria:10,Carisma:12},tools:[],featSpells:[],featSpellcasting:[],featResources:[],featMechanics:{combatFlags:[]},weapon:state.catalogs.weapons[0],attack:5,ac:12,armor:null,unarmedDamage:null,metamagicOptions:[],metamagicAdeptSorceryPoints:0,invocationFlags:[],invocationSpells:[],invocationSenses:{},cantripInvocationModifiers:{},speed:30};
applyTashaFeatEffects(d);assert.ok(d.tools.includes('Ferramentas de Ferreiro'));assert.deepEqual(new Set(d.featSpells.map(x=>x.id)),new Set(['mending','cure-wounds']));assert.ok(d.featSpellcasting.some(x=>x.ability==='Inteligência'));assert.ok(d.legacyInvocationEffects.some(x=>x.source==='Eldritch Adept'&&x.spellGrants.some(g=>g.spell?.id==='mage-armor'&&g.noSlot&&g.usage==='à vontade')),'Armor of Shadows via Eldritch Adept deve conceder Mage Armor à vontade e sem espaço.');assert.equal(d.attack,7,'Archery de Fighting Initiate deve acrescentar +2 ao ataque à distância.');assert.ok(d.fightingStyles.includes('Archery'));assert.deepEqual(new Set(d.metamagicOptions),new Set(['careful-spell','twinned-spell']));assert.equal(d.metamagicAdeptSorceryPoints,2);assert.equal(d.sorceryPoints,undefined,'Metamagic Adept não pode engrossar o pool normal de Pontos de Feitiçaria.');assert.ok(d.featResources.some(x=>x.label.includes('Metamagic Adept')&&x.max===2));
