...
import{readCollaborationSession,sharedCampaignRows,sharedAdventures,sharedParticipants,participantsForSession,participantsForAdventure,assignedCharacterIds,ownedCharacterIds,editableCharacterIds,visibleCharacterIds,canOpenCharacter,canEditCharacter,collaborationAccessMode}from'../scripts/collaboration-view.js';
...
storage.setItem('hub-rpg:characters:v4',JSON.stringify([{id:'pc-gus',name:'Ficha do Gus',ownerUid:'u-gus',ownerUsername:'gus'},{id:'pc-novo',name:'Personagem novo',ownerUid:'u-gus',ownerUsername:'gus'},{id:'pc-leo',name:'Ficha do Leo',ownerUid:'u-leo',ownerUsername:'leo'}]));
storage.setItem('hub-rpg:collaboration-cache:v1',JSON.stringify({campaigns:{
 c1:{
  membership:{campaignId:'c1',username:'gus',email:'gus@teias.invalid',role:'player',characterId:'pc-gus',sessionIds:['s1'],adventureIds:['a1'],active:true},
  payload:{
   campaign:{id:'c1',name:'Mesa Gus',members:[{id:'m-gus',name:'Gus',role:'player',characterId:'pc-gus',active:true},{id:'m-leo',name:'Leo',role:'player',characterId:'pc-leo',active:true}],sessions:[{id:'s1'}]},
   participants:[{username:'gus',role:'player',characterId:'pc-gus',sessionIds:['s1'],adventureIds:['a1'],active:true},{username:'leo',role:'player',characterId:'pc-leo',sessionIds:['s1'],adventureIds:['a1'],active:true}],
   revealedAdventures:[{id:'a1',campaignId:'c1',title:'Aventura Gus'}]
  },
  characterIds:['pc-gus','pc-leo']
 },
 c2:{
  membership:{campaignId:'c2',username:'leo',email:'leo@teias.invalid',role:'player',characterId:'pc-leo-2',active:true},
  payload:{campaign:{id:'c2',name:'Mesa Leo'},participants:[],revealedAdventures:[]},
...
assert.deepEqual(ownedCharacterIds(storage).sort(),['pc-gus','pc-novo'].sort());
assert.deepEqual(editableCharacterIds(storage).sort(),['pc-gus','pc-novo'].sort());
assert.deepEqual(visibleCharacterIds(storage).sort(),['pc-gus','pc-leo','pc-novo'].sort());
...
assert.equal(canOpenCharacter('pc-novo',storage),true,'Jogador deve abrir personagem próprio mesmo antes de atribuição a uma Campanha.');
assert.equal(canEditCharacter('pc-novo',storage),true,'Jogador deve editar personagem próprio criado pela conta.');
...
assert.equal(collaborationAccessMode(guestStorage),'guest');
assert.equal(canOpenCharacter('pc-gus',guestStorage),false);
assert.equal(canEditCharacter('pc-gus',guestStorage),false);
...
const files={
 ...
 wizard:fs.readFileSync(new URL('../scripts/character-builder/wizard-ui.js',import.meta.url),'utf8'),
 sync:fs.readFileSync(new URL('../scripts/collaboration-sync.js',import.meta.url),'utf8'),
 realtime:fs.readFileSync(new URL('../scripts/collaboration-realtime.js',import.meta.url),'utf8'),
...
assert.ok(files.characters.includes('editableCharacterIds')&&files.characters.includes('Seu personagem')&&files.characters.includes('Outro jogador')&&files.characters.includes('hidden=guest'));
...
assert.ok(files.wizard.includes('if(!id)return true')&&files.wizard.includes('stampPlayerOwnership'),'Criação nova de Jogador autenticado não pode ser redirecionada e deve registrar a conta proprietária.');
assert.ok(files.sync.includes('ownerUid')&&files.sync.includes('ownerUsername')&&files.sync.includes('isPlayerAccount(account,provider)'),'Sincronização deve reconhecer personagem próprio ainda não atribuído.');
assert.ok(files.realtime.includes('editableCharacterIds')&&files.realtime.includes('saveOwnCharacter'),'Tempo real deve persistir personagens editáveis da própria conta.');
...
console.log('OK — login define a visão; Jogador cria e edita a própria ficha, lê co-participantes do mesmo contexto e não edita conteúdo alheio.');