...
const pages=['index.html','personagens.html','lista-personagens.html','criacao-personagem.html','ficha-personagem.html','campanhas.html','mesa.html','sessoes.html','aventuras.html','usuarios.html','painel.html','bibliotecas.html','configuracoes.html'];
...
assert.ok(listUi.includes('.hidden=guest')&&listUi.includes('canEditCharacter(c.id)')&&listUi.includes('const remove=!ctx.restricted&&!info')&&listUi.includes('ainda não vinculado'),'Jogador autenticado deve editar apenas fichas autorizadas; fichas pessoais de outros jogadores permanecem sem exclusão e o Mestre enxerga as ainda não vinculadas em leitura.');
