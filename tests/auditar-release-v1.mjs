...
for(const path of['README.md','ROADMAP-V1.md','FIREBASE-PROVISIONAMENTO.md','docs/MANUAL.md','docs/ARQUITETURA.md','package.json','playwright.config.js','e2e/hub.spec.js','e2e/player-visibility.spec.js','e2e/collaboration-real.spec.js','tests/auditar-acesso-jogador-ui.mjs','tests/auditar-desempenho-ui.mjs','.github/workflows/homologar-release-v1.yml','.github/workflows/homologar-firebase-real.yml','.github/workflows/publicar-v1-0-2.yml'])assert.equal(exists(path),true,`Versão atual sem artefato obrigatório: ${path}`);
...
for(const token of['membership-character','membership-sessions','membership-adventures','master-publish-tools'])assert.ok(users.includes(token),`Área Jogadores sem ${token}.`);
...
const critical=['tests/auditar-progressao-level.mjs','tests/auditar-campanhas-mesas.mjs','tests/auditar-aventuras.mjs','tests/auditar-colaboracao-provisionamento.mjs','tests/auditar-colaboracao-sync.mjs','tests/auditar-acesso-jogador-ui.mjs','tests/auditar-painel-geral.mjs','tests/auditar-configuracoes.mjs','tests/auditar-ux-mobile-final.mjs','tests/auditar-desempenho-ui.mjs'];
...
console.log('OK — gate atual: login simples, Rafael + Jogadores, acessos explícitos, UI restrita, ficha própria e desempenho validados.');
