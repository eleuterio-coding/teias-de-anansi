...
const mediaAccess=rules.slice(rules.indexOf('function canReadMedia'),rules.indexOf('match /users/{uid}'));
assert.ok(mediaAccess.includes('allow create: if signedIn()'));
assert.ok(mediaAccess.includes('request.resource.data.campaignId == null'));
assert.ok(mediaAccess.includes('canManage(request.resource.data.campaignId)'),'Imagem vinculada à Campanha só pode ser gravada por quem administra a Campanha.');
assert.ok(mediaAccess.includes("data.entityType == 'campaign' && isMember(data.campaignId)"),'Membro ativo deve conseguir ler a capa da Campanha.');
assert.ok(mediaAccess.includes("data.entityType == 'adventure' && assignedAdventure(data.campaignId, data.entityId)"),'Jogador deve ler apenas capas de Aventuras atribuídas.');
assert.ok(mediaAccess.includes("data.entityType == 'session' && assignedSession(data.campaignId, data.entityId)"),'Jogador deve ler apenas capas de Sessões atribuídas.');
