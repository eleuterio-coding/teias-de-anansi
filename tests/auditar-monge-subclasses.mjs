...
{
 const out=monkSubclassOutcome(make('Way of the Sun Soul'));assert.ok(out.attacks.some(x=>x.name==='Radiant Sun Bolt'&&x.attackBonus===11&&x.damage==='d12 + 5 Radiant'),'Radiant Sun Bolt deve usar DEX.');assert.ok(out.focusTechniques.some(x=>x.name==='Searing Sunburst'&&/8d6/.test(x.effect)&&/CD 18/.test(x.effect)),'Searing Sunburst incompleto.');assert.ok(out.summary.some(x=>x.name==='Sun Shield — Retaliação'&&/9 Radiant/.test(x.value)),'Sun Shield deve causar 5 + SAB.')
}
{
