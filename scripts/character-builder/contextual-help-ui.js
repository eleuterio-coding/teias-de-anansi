...
const BROWSABLE=new Set(['class','subclass','background','species','lineage','feat','armor','weapon']);
...
function kindForSelect(select){
 ...
 if(select.classList.contains('feat-select')||select.id==='bg-origin-feat')return'feat';
...
function decorate(){scheduled=false;document.querySelectorAll('#classe,#subclasse,#antecedente,#especie,#sp-line,#bg-origin-feat,.feat-select,#armor,#weapon,select[data-feat-list="spell"],select[data-feat-simple]').forEach(decorateSelect);decorateSpells()}
