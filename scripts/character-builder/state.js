export const KEY='hub-rpg:characters:v4',LEGACY_KEYS=['hub-rpg:characters:v3','hub-rpg:characters:v2','hub-rpg:characters:v1'],V='20260823-character-builder5',PIN='bfd3db4bcc31699cce703b46feb9af3f0ff08999';
...
export function loadCharacter(){const id=new URLSearchParams(location.search).get('id'),saved=id?read().find(x=>x.id===id):null,c=saved?structuredClone(saved):blank(),wasLegacyRules=c.ruleset&&c.ruleset!=='5.5e',oldClass=c.choices?.class||{},oldSpells=c.choices?.spells||{},oldBg=c.choices?.background||{};c.schema='hub-rpg/personagem/v4';c.ruleset='5.5e';c.refs={class:null,species:null,background:null,subclass:null,...(c.refs||{})};if(wasLegacyRules)c.refs={class:null,species:null,background:null,subclass:null};c.refSnapshots=c.refSnapshots||{};c.baseAbilities={...Object.fromEntries(AB.map(a=>[a,10])),...(c.baseAbilities||{})};c.choices=c.choices||{};c.choices.class={level:Math.max(1,Math.min(20,num(oldClass.level)||1)),skills:arr(oldClass.skills)};c.choices.species={size:null,lineage:null,...(c.choices.species||{})};c.choices.background={abilityMode:'2+1',plus2:null,plus1:null,plusOnes:[],equipment:'A',toolChoice:'',...oldBg,plusOnes:arr(oldBg.plusOnes)};c.choices.feats=c.choices.feats||{};c.choices.equipment={armor:null,shield:false,weapon:null,...(c.choices.equipment||{})};c.choices.spells={cantrips:arr(oldSpells.cantrips),leveled:arr(oldSpells.leveled),arcanum:{...(oldSpells.arcanum||{})}};return c}
export function snapshot(x){return x?{id:x.id,name:x.name,ruleset:'5.5e',revision:2024,source:x.source||x.fonte||''}:null}
