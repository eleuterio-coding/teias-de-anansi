...
export function activeFeatInstances(){
 ...
 const special=backgroundSpecialFeatInstance();if(special)out.push(special);
...
function spellAllowed(spell,def,instanceData){if(!spell)return false;if(def.level!=null&&num(spell.level)!==num(def.level))return false;if(def.type==='ritual_spells'&&!spell.ritual)return false;if(def.schools?.length&&!def.schools.some(s=>fold(s)===fold(spell.school)))return false;if(def.spellClasses?.length){const wanted=arr(def.spellClasses).map(fold);if(!arr(spell.classes).some(c=>wanted.includes(fold(c))))return false}if(def.listFrom){const list=instanceData?.[def.listFrom];if(!list)return false;const aliases={clerigo:['clerigo','cleric'],druida:['druida','druid'],mago:['mago','wizard']},allowed=aliases[fold(list)]||[fold(list)];if(!arr(spell.classes).some(c=>allowed.includes(fold(c))))return false}return true}

export function sanitizeFeatChoices(){
 if(!state.c?.choices)return;sanitizeBackgroundSpecialFeatChoice();sanitizeClassFeatureFeatSelections();
...
function instanceOutcome(inst,baseSkills,baseSaves){
 const rule=featRule(inst.feat)||{},data=store()[inst.key]||{},abilityBonuses={},abilityCaps={},skills=[],expertise=[],skillUpgrades=[],tools=[...arr(rule.fixedTools)],saveProficiencies=[],spells=[],labels=[],extraProficiencies=[...arr(rule.extraProficiencies)],spellAbility=rule.spellAbilityFixed||data.spellAbility||data.ability||null;
