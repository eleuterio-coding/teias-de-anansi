...
function checkbox(spell,kind,checked,disabled=false,note=''){return`<label class="check spell-check"><input type="checkbox" data-progression-kind="${kind}" value="${esc(spell.id)}" ${checked?'checked':''} ${disabled?'disabled':''}><span><strong>${esc(spell.name)}</strong>${spell.school?` <small class="muted">· ${esc(spell.school)}</small>`:''}${note?` <small class="muted">· ${esc(note)}</small>`:''}</span></label>`}
...
function visualRows(visual,level){return{active:visual?.activeByLevel?.get(level)||new Set(),removed:visual?.removedByLevel?.get(level)||new Map(),swapIn:visual?.swapInByLevel?.get(level)||new Set()}}
function renderVisualCheckbox(spell,kind,selectedIds,full,visual){
 const replacedAt=visual.removed.get(spell.id),swapIn=visual.swapIn.has(spell.id),checked=visual.active.has(spell.id),note=swapIn?'selecionada por substituição':replacedAt?`substituída no nível ${replacedAt}`:'',disabled=swapIn||!!replacedAt||(full&&!selectedIds.includes(spell.id));
 return checkbox(spell,kind,checked,disabled,note)
}
function renderCantripGain(klass,step,visualMap){
 const visual=visualRows(visualMap,step.level);if(!step.cantripGain&&!visual.swapIn.size)return'';
 const selectedIds=arr(step.stored.cantrips);let candidates=spellProgressionCandidates(klass,step.level,{kind:'cantrip'}).filter(matchesSearch);if(!step.cantripGain)candidates=candidates.filter(spell=>visual.swapIn.has(spell.id));const full=selectedIds.length>=step.cantripGain,title=step.cantripGain?`Novos truques — ${selectedIds.length}/${step.cantripGain}`:'Truque selecionado por substituição';
 return`<section><h4>${esc(title)}</h4><p class="mini">As marcações mostram o repertório efetivo. Uma substituição desmarca o truque removido e marca o novo.</p><div class="check-grid spell-grid">${candidates.map(spell=>renderVisualCheckbox(spell,'gain-cantrip',selectedIds,full,visual)).join('')||'<p class="muted">Nenhum truque corresponde à busca.</p>'}</div></section>`
}
function renderLeveledGain(klass,step,visualMap){
 const visual=visualRows(visualMap,step.level);if(!step.leveledGain&&!visual.swapIn.size)return'';
 const selectedIds=arr(step.stored.leveled),before=new Set(step.beforeLeveled);let candidates=spellProgressionCandidates(klass,step.level,{kind:'leveled'}).filter(spell=>(!before.has(spell.id)||visual.swapIn.has(spell.id))&&matchesSearch(spell));if(!step.leveledGain)candidates=candidates.filter(spell=>visual.swapIn.has(spell.id));const full=selectedIds.length>=step.leveledGain,levels=[...new Set(candidates.map(spell=>spell.level))].sort((a,b)=>a-b),title=step.leveledGain?`Novas magias — ${selectedIds.length}/${step.leveledGain}`:'Magia selecionada por substituição';
 return`<section><h4>${esc(title)}</h4><p class="mini">Neste nível, você pode escolher qualquer magia de círculo permitido; não é obrigatório escolher o círculo mais alto. As marcações refletem o repertório efetivo após substituições.</p>${levels.map(circle=>{const rows=candidates.filter(spell=>spell.level===circle),open=rows.some(spell=>visual.active.has(spell.id)||visual.removed.has(spell.id));return`<details ${open?'open':''}><summary><strong>${circle}º círculo</strong></summary><div class="check-grid spell-grid">${rows.map(spell=>renderVisualCheckbox(spell,'gain-leveled',selectedIds,full,visual)).join('')}</div></details>`}).join('')||'<p class="muted">Nenhuma magia corresponde à busca.</p>'}</section>`
}
