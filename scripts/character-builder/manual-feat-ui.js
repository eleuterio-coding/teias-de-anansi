...
function decorate(){
 ...
 box.querySelector('[data-manual-feats-note]')?.remove();const manual=manualInstances();if(!manual.length)return;
 const note=document.createElement('div');note.dataset.manualFeatsNote='';note.className='status warning';note.innerHTML=`<strong>Talento com aplicação manual</strong><p class="mini">${manual.map(x=>x.feat.name).join(', ')} está disponível no catálogo, mas o Hub possui apenas uma síntese ou bloco insuficiente para automatizar todos os efeitos com segurança. A escolha é salva na ficha; aplique a mecânica integral da fonte sem inferir valores ausentes.</p>`;box.appendChild(note)
}
