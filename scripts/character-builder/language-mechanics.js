...
function structuredFeatLanguageDefinitions(stages){const defs=[];for(const feat of uniq([...stages.origin,...stages.species,...stages.progression])){const name=fold(feat?.name),prefix=`feat:${feat.id}:structured-language`;if(name==='fey teleportation')defs.push({key:`${prefix}:sylvan`,label:`${feat.name} — idioma`,fixed:['Silvestre'],choose:0,pool:[]});if(name==='prodigy')defs.push({key:`${prefix}:choice`,label:`${feat.name} — idioma`,fixed:[],choose:1,pool:ALL_LANGUAGES})}return defs}
...
export function languageGrantDefinitions(){
 ...
 const originSources=[];if(bg?.feature)originSources.push([`background:${bg.id}:feature`,`${bg.name} — característica`,bg.feature.text||bg.feature.description||'']);const stages=featStages();defs.push(...structuredFeatLanguageDefinitions(stages));for(const feat of stages.origin)originSources.push([`feat:origin:${feat.id}`,`Talento de Origem — ${feat.name}`,feat.description||'']);appendTextSources(defs,originSources);
