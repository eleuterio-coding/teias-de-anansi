...
function subclassLanguageDefinitions(sub){const name=fold(sub?.mechanics?.name||sub?.name||'');if(name!==fold('Path of the Demonshard'))return[];const value=state.c.choices?.subclassMechanics?.['path-of-the-demonshard']?.demontongue;if(!['Abissal','Infernal'].includes(value))return[];return[{key:'subclass:demonshard:demontongue',label:`${sub.name} — Demontongue`,fixed:[value],choose:0,pool:[]}]}
