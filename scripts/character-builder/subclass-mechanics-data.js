...
export function initSubclassMechanicsData(){
 ...
 pending=(async()=>{
  ...
  const setPreferred=(key,value)=>{const current=map.get(key);if(!current||rank(value.fonte_id)>rank(current.fonte_id))map.set(key,value)};
  ...
  for(const sub of arr(state.catalogs.subclasses)){
   const row=map.get(fold(sub.originalName||sub.name));if(!row)continue;
