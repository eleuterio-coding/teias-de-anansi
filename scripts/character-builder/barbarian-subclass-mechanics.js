...
export function barbarianSubclassChoiceDefs(d){
 ...
 if(n===fold('Path of the Wild Heart')){
  ...
  add('drinkingBuddies',10,'Bebida atual de Drinking Buddies',['Ginger Ale','Pulque','Warm Cider'],'durante Descanso Curto ou Longo',false,'Representa a preparação atualmente distribuída ao grupo.')
 }
 return defs
}
...
export function barbarianSubclassOutcome(d){
 if(!isBarbarian(d))return null;const l=num(d.level),pb=num(d.pbonus),str=mod(d.scores?.Força),con=mod(d.scores?.Constituição),n=fold(canonical(d)),{values,pending}=sanitizeBarbarianSubclassChoices(d),features=arr(d.sub?.features).filter(f=>num(f.level)<=l),summary=[],resources=[],defenses=[],attacks=[],movementModes={},senses=[],tools=[],languages=[];
 ...
 const attack=(name,damage,extra='')=>attacks.push({name,attackBonus:str+pb,damage,extra});
 ...
 if(n===fold('Path of the Berserker')){
  ...
  if(l>=14){add('CD de Intimidating Presence',strDC,'8 + mod. Força + PB');resource('Intimidating Presence',1,'Descanso Longo','Pode restaurar o uso gastando uma Fúria.')}
  ...
  if(l>=6)add('CD de Branches of the Tree',strDC,'8 + mod. Força + PB');
  ...
  if(l>=10)resource('Zealous Presence',1,'Descanso Longo','Pode restaurar o uso gastando uma Fúria.');
  ...
  const f=values.beastForm;if(f==='Mordida')attack('Mordida','1d8 + mod. Força Perfurante',`1/turno abaixo de metade dos PV: cura ${pb} PV`);if(f==='Garras')attack('Garras','1d6 + mod. Força Cortante','1/turno: ataque de garra adicional na ação Atacar');if(f==='Cauda')attack('Cauda','1d8 + mod. Força Perfurante','Reach; Reação: 1d8 à CA contra ataque que acertaria');
  if(l>=6){add('Bestial Soul','Armas naturais contam como mágicas');if(values.bestialSoul==='Natação')movementModes.swim=d.speed;if(values.bestialSoul==='Escalada')movementModes.climb=d.speed;if(values.bestialSoul==='Salto')add('Bestial Soul — Salto','Saltos ampliados por teste de Atletismo','até a próxima escolha após descanso')}
  if(l>=10){add('CD de Infectious Fury',conDC,'8 + mod. Constituição + PB');resource('Infectious Fury',pb,'Descanso Longo','Ao acertar com arma natural: ataque reativo do alvo ou 2d12 Psychic.')}
  if(l>=14){resource('Call the Hunt',pb,'Descanso Longo',`Até ${Math.max(1,con)} criatura(s); você ganha 5 PV temporários por criatura e cada aliado afetado causa +1d6 1/turno.`)}
 }else if(n===fold('Path of Wild Magic')){
  resource('Magic Awareness',pb,'Descanso Longo','Detecta magias e itens mágicos a 60 ft até o fim do próximo turno.');add('CD de Wild Surge',conDC,'8 + mod. Constituição + PB');
  if(l>=6)resource('Bolstering Magic',pb,'Descanso Longo','1d3 em ataques/testes por 10 min ou recuperação de slot conforme 1d3.');
  if(l>=10)add('Unstable Backlash','Rola novamente Wild Surge como Reação','ao sofrer dano ou falhar em salvaguarda durante a Fúria');
  ...
  add('Ancestral Protectors','Primeiro inimigo acertado no turno fica pressionado pelos espíritos','durante a Fúria');
  ...
  const env=values.stormAura;add('CD de Storm Aura',conDC,'8 + mod. Constituição + PB');
  if(env==='Deserto')add('Storm Aura — Deserto',`${stormFlat(l)} de dano de Fogo`,'todas as outras criaturas na aura de 10 ft');
  if(env==='Mar')add('Storm Aura — Mar',`${stormSea(l)} de dano de Lightning`,'uma criatura na aura; teste de Destreza, metade no sucesso');
  if(env==='Tundra')add('Storm Aura — Tundra',`${stormFlat(l)} PV temporários`,'criaturas escolhidas na aura');
  if(l>=6&&env==='Deserto')defense('Storm Soul — Deserto','Resistência a Fogo','permanente');
  ...
  if(l>=6&&env==='Tundra')defense('Storm Soul — Tundra','Resistência a Frio','permanente');
  if(l>=10)add('Shielding Storm','Compartilha a resistência de Storm Soul com criaturas escolhidas na aura','durante a Fúria');
  if(l>=14)add('Raging Storm',env?`Efeito avançado de ${env}`:'Efeito depende do ambiente escolhido','durante a Fúria')
 }else if(n===fold('Path of the Demonshard')){
  if(values.demontongue)languages.push(values.demontongue);add('Fiendish Punishment',`${demonDie(l)} ${values.punishmentType||'Fogo/Necrótico'}`,'1/turno durante a Fúria; também pode punir agressor de aliado por Reação');
  if(l>=6){defense('Just One Master','Resistência a Psychic','durante a Fúria');add('Just One Master','Vantagem em salvaguardas contra Enfeitiçado e Amedrontado','durante a Fúria')}
  if(l>=10)add('Overflowing Malice',`Espalha os mesmos ${demonDie(l)} para criaturas a 10 ft do alvo`);
  if(l>=14)add('CD de Aspect of Tyranny',conDC,'8 + mod. Constituição + PB; -1 por outra criatura afetada pela mesma punição')
 }else if(n===fold('Path of the Brewmaster')){
  tools.push('Suprimentos de Cervejeiro');add('Brewer',`+${l>=10?2:1} em ataque e dano com armas improvisadas`,l>=6?'ataques contam como mágicos':'');resource('Ancestral Spirits',Math.min(6,rages),'Descanso Longo','Quantidade de bebidas preparadas acompanha o máximo de usos de Fúria, até 6.');
  const drink=values.ancestralDrink,stout=l>=10?'1d8':'1d4';if(drink==='Kefir')add('Kefir','Vantagem em salvaguardas de Constituição','durante a Fúria');if(drink==='Kombucha')add('Kombucha','Reação adicional por rodada somente para Ataque de Oportunidade','durante a Fúria');if(drink==='Mead')add('Mead','+10 ft de deslocamento e OA contra você com Desvantagem','durante a Fúria');if(drink==='Sake')add('Sake','Vantagem em salvaguardas de Sabedoria','durante a Fúria');if(drink==='Stout')add('Stout',`${stout} PV temporários no início do turno; abaixo de metade dos PV, cura ${stout}`,'durante a Fúria');
  if(l>=6&&values.brewersGut==='Beer Jacket')defense("Brewer's Gut — Beer Jacket",'Resistência a Frio','até a próxima escolha após descanso');if(l>=6&&values.brewersGut==='Guts of Steel')defense("Brewer's Gut — Guts of Steel",'Resistência a Veneno','até a próxima escolha após descanso');if(l>=6&&values.brewersGut==='Liquid Courage')defense("Brewer's Gut — Liquid Courage",'Imunidade a Amedrontado','até a próxima escolha após descanso');
  if(l>=10){const buddy=values.drinkingBuddies;if(buddy==='Ginger Ale')add('Drinking Buddies — Ginger Ale','Vantagem em Iniciativa');if(buddy==='Pulque')add('Drinking Buddies — Pulque',`+${Math.max(1,con)} em testes de Carisma`);if(buddy==='Warm Cider')add('Drinking Buddies — Warm Cider','3 PV temporários no início de cada turno')}
  if(l>=14)resource('Fermented Binge',1,'Descanso Curto ou Longo','Intensifica a bebida de Ancestral Spirits usada ao entrar em Fúria.')
 }
 return{name:canonical(d),level:l,rageDamage:rDamage,rages,choices:values,choiceDefs:barbarianSubclassChoiceDefs(d),pending,features,summary,resources,defenses,attacks,movementModes,senses,tools:uniq(tools),languages:uniq(languages)}
}

export function applyBarbarianSubclassMechanics(d){
 const out=barbarianSubclassOutcome(d);if(!out)return d;d.subclassFeatures=out.features;d.subclassMechanics=out;d.tools=uniq([...arr(d.tools),...out.tools]);d.subclassAttacks=out.attacks;d.subclassResources=out.resources;d.subclassDefenses=out.defenses;d.subclassMovementModes={walk:d.speed,...out.movementModes};d.subclassSenses=out.senses;d.subclassLanguages=out.languages;return d
}
