...
function markCurrent(link,links){for(const a of links)a.removeAttribute('aria-current');if(link){link.setAttribute('aria-current','location');link.scrollIntoView({block:'nearest',inline:'center'})}}
function enhanceSectionNav(){
 observer?.disconnect();observer=null;
 const nav=document.querySelector('.section-nav');if(!nav)return;nav.setAttribute('aria-label',nav.getAttribute('aria-label')||'Navegação por seções');nav.dataset.horizontalScroll='true';
 const links=sectionLinks(nav);if(!links.length)return;
 for(const link of links)link.addEventListener('click',()=>markCurrent(link,links));
 ...
 const hashLink=links.find(a=>a.hash===location.hash)||links[0];if(hashLink)markCurrent(hashLink,links)
}
function enhanceTouchGroups(){
 for(const group of document.querySelectorAll('.hero .actions,.top .actions,.hero-actions'))group.dataset.mobileStack='true'
}
...
if(typeof document!=='undefined'){
 ...
 const mo=new MutationObserver(()=>queueMicrotask(()=>{enhanceLiveRegions();enhanceTouchGroups();enhanceStructureEditingCopy();if(document.querySelector('.section-nav'))enhanceSectionNav()}));
