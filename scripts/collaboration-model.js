...
export function normalizeMembership(row={}){return{id:text(row.id),campaignId:text(row.campaignId),uid:text(row.uid),role:role(row.role),characterId:text(row.characterId)||null,sessionIds:unique(row.sessionIds),adventureIds:unique(row.adventureIds),displayName:text(row.displayName),active:row.active!==false,updatedAt:text(row.updatedAt)}}
