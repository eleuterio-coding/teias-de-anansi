...
export function normalizeAuthorizedUser(row={}){return{uid:text(row.uid||row.id),username:normalizeUsername(row.username),active:row.active===true,isAdmin:row.isAdmin===true}}
export function normalizeAccount(user={},authorization={}){const a=normalizeAuthorizedUser({...authorization,uid:authorization.uid||user.uid}),username=a.username||usernameFromTechnicalEmail(user.email);return{uid:text(user.uid),username,displayName:username||'Usuário',active:a.active,isAdmin:a.isAdmin}}
