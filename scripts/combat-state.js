...
export function initiativeOrder(combat){return arr(combat?.combatants).map(sanitizeCombatant).sort((a,b)=>{const ai=a.initiative==null?-Infinity:a.initiative,bi=b.initiative==null?-Infinity:b.initiative;return bi-ai||b.initiativeModifier-a.initiativeModifier||a.name.localeCompare(b.name,'pt-BR')||a.id.localeCompare(b.id)})}
export function sortCombatInitiative(combat){if(!combat)return[];combat.combatants=initiativeOrder(combat);return combat.combatants}
export function setCombatantInitiative(combat,combatantId,value){return updateCombatant(combat,combatantId,{initiative:Number.isFinite(Number(value))?Number(value):null},{sort:true})}
export function startCombat(session,combatId){
 if(!session)return{ok:false,reason:'Sessão indisponível.'};const fields=sanitizeCombatSessionFields(session),combat=fields.combats.find(c=>c.id===text(combatId));if(!combat)return{ok:false,reason:'Combate não encontrado.'};const other=fields.combats.find(c=>c.status==='active'&&c.id!==combat.id);if(other)return{ok:false,reason:`Finalize o combate ativo (${other.title}) antes de iniciar outro.`};if(!combat.combatants.length)return{ok:false,reason:'Adicione ao menos um combatente antes de iniciar.'};combat.status='active';combat.round=1;combat.startedAt=combat.startedAt||now();combat.endedAt=null;sortCombatInitiative(combat);combat.activeCombatantId=combat.combatants[0]?.id||null;if(combat.combatants[0])combat.combatants[0].reactionAvailable=true;combat.updatedAt=now();session.combats=fields.combats;session.activeCombatId=combat.id;return result(session,combat)
}
...
export function addCombatant(combat,data={}){
 if(!combat)return{ok:false,reason:'Combate indisponível.'};if(data.kind==='character'&&data.characterId&&combat.combatants.some(c=>c.kind==='character'&&c.characterId===data.characterId))return{ok:false,reason:'Personagem já está no combate.'};const combatant=sanitizeCombatant({...data,id:data.id||uid(data.kind||'combatant')});combat.combatants.push(combatant);combat.updatedAt=now();return{ok:true,combat,combatant}
}
...
export function removeCombatant(combat,combatantId){if(!combat)return{ok:false,reason:'Combate indisponível.'};const id=text(combatantId),before=combat.combatants.length;combat.combatants=combat.combatants.filter(c=>c.id!==id);if(combat.combatants.length===before)return{ok:false,reason:'Combatente não encontrado.'};if(combat.activeCombatantId===id)combat.activeCombatantId=combat.combatants[0]?.id||null;combat.updatedAt=now();return{ok:true,combat}}
export function reorderCombatant(combat,combatantId,direction=0){sortCombatInitiative(combat);const index=combat.combatants.findIndex(c=>c.id===text(combatantId)),target=index+(direction<0?-1:1);if(index<0||target<0||target>=combat.combatants.length)return{ok:false,reason:'Não é possível mover o combatente nessa direção.'};const [row]=combat.combatants.splice(index,1);combat.combatants.splice(target,0,row);combat.updatedAt=now();return{ok:true,combat}}
export function setGroupInitiative(combat,combatantId,value){const source=combat?.combatants?.find(c=>c.id===text(combatantId));if(!source)return{ok:false,reason:'Combatente não encontrado.'};const ids=source.groupInitiativeKey?combat.combatants.filter(c=>c.groupInitiativeKey===source.groupInitiativeKey).map(c=>c.id):[source.id];for(const id of ids){const row=combat.combatants.find(c=>c.id===id);if(row)row.initiative=Number.isFinite(Number(value))?Number(value):null}sortCombatInitiative(combat);combat.updatedAt=now();return{ok:true,combat,count:ids.length}}
