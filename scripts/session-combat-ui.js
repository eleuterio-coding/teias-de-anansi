...
function combatantHtml(c,combat,index=0){
 const active=c.id===combat.activeCombatantId,hpKnown=c.maxHp>0,kind=c.kind==='character'?'Personagem':c.kind==='monster'?'Monstro':'NPC';
 const death=c.kind==='character'&&c.deathState?'<span class="mini combat-death-state">Morte: '+c.deathState.successes+'✓ / '+c.deathState.failures+'✕'+(c.deathState.stable?' · estável':'')+(c.deathState.dead?' · morto':'')+'</span>':'';
 const stateBadges=(c.concentration?'<span class="badge">Concentração</span>':'')+(!c.reactionAvailable?'<span class="badge">Reação usada</span>':'');
 return'<div class="combat-table-row '+(active?'is-turn ':'')+(c.defeated?'defeated':'')+'" role="row" data-combatant="'+esc(c.id)+'">'+
 '<div class="combat-order-cell" role="cell"><button type="button" class="combat-drag-handle" draggable="true" data-drag-handle aria-label="Arrastar '+esc(c.name)+' para alterar a ordem de iniciativa" title="Arraste para mudar a ordem">⋮⋮</button><strong class="combat-order-number">'+(index+1)+'</strong></div>'+
 '<div class="combat-name-cell" role="cell"><strong>'+esc(c.name)+'</strong><span class="badge">'+kind+'</span>'+(active?'<span class="badge active">Turno</span>':'')+'<span class="combat-row-source">'+esc(c.source||'')+'</span></div>'+
 '<label class="combat-compact-input" role="cell"><span class="sr-only">Iniciativa de '+esc(c.name)+'</span><input type="number" data-combat-init value="'+(c.initiative??'')+'" aria-label="Iniciativa de '+esc(c.name)+'"></label>'+
 '<label class="combat-compact-input" role="cell"><span class="sr-only">CA de '+esc(c.name)+'</span><input type="number" min="0" data-combat-ac value="'+(c.ac??'')+'" aria-label="CA de '+esc(c.name)+'"></label>'+
 '<div class="combat-pv-cell" role="cell"><strong>'+c.currentHp+(hpKnown?'/'+c.maxHp:'')+'</strong>'+(c.tempHp?'<span class="combat-temp-hp">+'+c.tempHp+' temp.</span>':'')+death+'</div>'+
 '<div class="combat-status-cell" role="cell">'+combatantConditions(c)+stateBadges+'</div>'+
 '<div class="combat-row-actions" role="cell"><button type="button" class="secondary combat-order-button" data-move-combatant="-1" aria-label="Mover '+esc(c.name)+' para cima">↑</button><button type="button" class="secondary combat-order-button" data-move-combatant="1" aria-label="Mover '+esc(c.name)+' para baixo">↓</button><button type="button" class="secondary" data-toggle-combatant-details>Detalhes</button></div>'+
 '<div class="combat-row-details" data-combatant-details hidden>'+
 '<div class="combat-detail-primary"><div class="combat-inline-controls"><button type="button" class="secondary" data-hp-delta="-5">−5 PV</button><button type="button" class="secondary" data-hp-delta="-1">−1 PV</button><button type="button" class="secondary" data-hp-delta="1">+1 PV</button><button type="button" class="secondary" data-hp-delta="5">+5 PV</button><label>Valor<input type="number" min="0" value="0" data-hp-amount></label><label>Tipo<select data-hp-type>'+damageOptions()+'</select></label><button type="button" data-damage>Aplicar dano</button><button type="button" class="secondary" data-heal>Curar</button><label>PV temp.<input type="number" min="0" value="'+c.tempHp+'" data-temp-hp></label><label>PV máximo<input type="number" min="0" value="'+(c.maxHp||'')+'" data-combat-max-hp></label></div></div>'+
 '<div class="combat-state-row"><div class="combat-state-buttons"><button type="button" class="'+(c.concentration?'':'secondary')+'" data-toggle-concentration>Concentração</button><button type="button" class="'+(c.reactionAvailable?'secondary':'')+'" data-toggle-reaction>'+(c.reactionAvailable?'Usar reação':'Restaurar reação')+'</button><button type="button" class="secondary" data-roll-d20>d20</button><button type="button" class="secondary" data-roll-init>Rolar iniciativa</button></div></div>'+
 ...
 actionButtons(c)+'<div class="combat-detail-footer"><label class="combatant-notes">Notas rápidas<input data-combatant-notes value="'+esc(c.notes||'')+'"></label><div class="combatant-actions">'+(c.kind==='character'?'<a class="btn secondary" href="ficha-personagem.html?id='+encodeURIComponent(c.characterId)+'">Abrir Ficha</a>':'<button type="button" class="secondary" data-target-combatant>Usar como alvo</button>')+'<button type="button" class="danger" data-remove-combatant>Remover</button></div></div></div></div>'
}
function combatTableHtml(combat){
 const ordered=initiativeOrder(combat);
 return'<div class="combat-table-wrap"><div class="combat-initiative-table" role="table" aria-label="Ordem de iniciativa">'+
 '<div class="combat-table-header" role="row"><div role="columnheader">Ordem</div><div role="columnheader">Personagem</div><div role="columnheader">Inic.</div><div role="columnheader">CA</div><div role="columnheader">PV</div><div role="columnheader">Estado</div><div role="columnheader">Ações</div></div>'+
 ordered.map((unit,index)=>combatantHtml(unit,combat,index)).join('')+'</div></div>'
}
function combatHeader(combat,ctx){
 ...
 '<div class="combat-main-toolbar"><button type="button" class="secondary" data-prev-combat-turn>← Turno anterior</button><button type="button" data-next-combat-turn>Próximo turno →</button><button type="button" class="secondary" data-sort-initiative>Ordenar pela iniciativa</button><button type="button" class="secondary" data-roll-all-enemies>Rolar inimigos</button><button type="button" class="secondary" data-refresh-characters>Atualizar fichas</button><span class="badge combat-order-mode">'+(combat.manualInitiativeOrder?'Ordem manual':'Ordem por iniciativa')+'</span><button type="button" class="danger" data-open-finish>Finalizar combate</button></div>'
}
...
function activeCombatHtml(combat,ctx){
 return'<article class="combat-workspace active" data-combat="'+esc(combat.id)+'">'+combatHeader(combat,ctx)+combatTableHtml(combat)+addCombatantsHtml(combat,ctx)+'<details class="combat-master-details"><summary>Notas do Mestre</summary><label>Notas do combate<textarea data-combat-notes>'+esc(combat.notes||'')+'</textarea></label><div class="row-actions"><button type="button" data-save-combat-notes>Salvar notas</button></div></details>'+finishPanel(combat)+'</article>'
}
function draftCombatHtml(combat,ctx){
 return'<article class="combat-history-card draft" data-combat="'+esc(combat.id)+'"><div class="combatant-head"><div><strong>'+esc(combat.title)+'</strong> <span class="badge">Preparado</span><div class="mini">'+esc(encounterName(ctx.session,combat.encounterId)||sceneName(ctx.session,combat.sessionSceneId)||'Combate rápido')+' · '+combat.combatants.length+' combatente(s)</div></div><div class="combatant-actions"><button type="button" data-start-combat>Iniciar combate</button><button type="button" class="secondary" data-open-draft>Editar preparação</button><button type="button" class="danger" data-remove-combat>Excluir</button></div></div><div data-draft-body hidden>'+combatTableHtml(combat)+addCombatantsHtml(combat,ctx)+'<label>Notas<textarea data-combat-notes>'+esc(combat.notes||'')+'</textarea></label><div class="row-actions"><button type="button" class="secondary" data-sort-initiative>Ordenar pela iniciativa</button><button type="button" data-save-combat-notes>Salvar preparação</button></div></div></article>'
}
...
function bindCombatantNode(ctx,combat,node){
 ...
 node.querySelector('[data-toggle-combatant-details]')?.addEventListener('click',()=>{const details=node.querySelector('[data-combatant-details]');if(details)details.hidden=!details.hidden});
 for(const button of node.querySelectorAll('[data-move-combatant]'))button.addEventListener('click',()=>{const r=reorderCombatant(combat,id,Number(button.dataset.moveCombatant));if(!r.ok)return announce(r.reason,false);persist(ctx,'Ordem de iniciativa atualizada.')});
 node.querySelector('[data-combat-init]')?.addEventListener('change',e=>{const r=unit.groupInitiativeKey?setGroupInitiative(combat,id,e.target.value):setCombatantInitiative(combat,id,e.target.value);if(!r.ok)return announce(r.reason,false);persist(ctx,'Iniciativa atualizada.')});node.querySelector('[data-combat-ac]')?.addEventListener('change',e=>{updateCombatant(combat,id,{ac:e.target.value});persist(ctx,'CA atualizada.')});node.querySelector('[data-combat-max-hp]')?.addEventListener('change',e=>{const max=Math.max(0,Number(e.target.value)||0),current=Math.min(max||unit.currentHp,unit.currentHp);updateCombatant(combat,id,{maxHp:max,currentHp:current});persist(ctx,'PV máximo atualizado.')});
...
function bindInitiativeDrag(ctx,node,combat){
 let draggedId=null;
 const clear=()=>{for(const row of node.querySelectorAll('[data-combatant]'))row.classList.remove('is-dragging','drop-before','drop-after')};
 for(const handle of node.querySelectorAll('[data-drag-handle]')){
  handle.addEventListener('dragstart',event=>{const row=handle.closest('[data-combatant]');draggedId=row?.dataset.combatant||null;if(!draggedId)return;event.dataTransfer?.setData('text/plain',draggedId);if(event.dataTransfer)event.dataTransfer.effectAllowed='move';row.classList.add('is-dragging')});
  handle.addEventListener('dragend',()=>{draggedId=null;clear()})
 }
 for(const row of node.querySelectorAll('[data-combatant]')){
  row.addEventListener('dragover',event=>{if(!draggedId||draggedId===row.dataset.combatant)return;event.preventDefault();clear();const rect=row.getBoundingClientRect(),after=event.clientY>rect.top+rect.height/2;row.classList.add(after?'drop-after':'drop-before');if(event.dataTransfer)event.dataTransfer.dropEffect='move'});
  row.addEventListener('drop',event=>{event.preventDefault();const source=event.dataTransfer?.getData('text/plain')||draggedId,target=row.dataset.combatant;if(!source||!target||source===target){clear();return}const rect=row.getBoundingClientRect(),placement=event.clientY>rect.top+rect.height/2?'after':'before',r=reorderCombatantTo(combat,source,target,placement);clear();draggedId=null;if(!r.ok)return announce(r.reason,false);persist(ctx,'Ordem de iniciativa atualizada.')})
 }
}
...
function bindCombatNode(ctx,node,combat){
 for(const unitNode of node.querySelectorAll('[data-combatant]'))bindCombatantNode(ctx,combat,unitNode);bindInitiativeDrag(ctx,node,combat);bindAddPanel(ctx,node,combat);
 node.querySelector('[data-save-combat-notes]')?.addEventListener('click',()=>saveCombatNotes(ctx,node,combat));
 node.querySelector('[data-sort-initiative]')?.addEventListener('click',()=>{sortCombatInitiative(combat);persist(ctx,'Ordem recalculada pela iniciativa.')});
 ...
 node.querySelector('[data-confirm-finish]')?.addEventListener('click',()=>{const outcome=node.querySelector('[data-combat-outcome]')?.value||'',notes=node.querySelector('[data-combat-notes]')?.value||combat.notes,history=node.querySelector('[data-finish-history]')?.checked!==false,sync=node.querySelector('[data-finish-sync]')?.checked!==false;combat.outcome=outcome;combat.notes=notes;if(history)appendCombatRecap(ctx.session,combat);const synced=sync?syncCharactersToSheets(ctx,combat):[];const r=finishCombat(ctx.session,combat.id,{outcome,notes});if(!r.ok)return announce(r.reason,false);persist(ctx,'Combate finalizado'+(synced.length?' · '+synced.length+' ficha(s) sincronizada(s).':'.'))})
}
