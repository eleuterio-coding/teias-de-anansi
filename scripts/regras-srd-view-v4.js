...
const ROUTES={
  ...
  'Monstros':'monstros.html','Armaduras':'armaduras.html','Armas':'armas.html','Equipamentos de Aventura':'equipamentos-aventura.html',
  'Ferramentas':'ferramentas.html','Montarias e Veículos':'montarias-veiculos.html','Comércio e Despesas':'comercio-e-despesas.html',
  'Bugigangas':'bugigangas.html','Itens Mágicos':'itens-magicos.html','Magias':'magias.html','Idiomas':'idiomas.html'
};
const SPECIAL_LABELS={'Armaduras::#phb-2024-shield':'Escudo'};
...
function extractEntries(markdown,knownOriginals){
  ...
  return starts.map((s,k)=>{
    ...
    return {heading:s.heading,original:s.base,tag:s.tag,markdown:lines.slice(s.i+1,end).join('\n').replace(/\n\* \* \*\s*$/,'').trim()};
  });
}
...
function fallbackRuleRow(original){
  const pt=ptRule(original);
  return {modulo:'Regras',original,nome:pt?.nome||original,id:null,url:'regras.html'};
}
function entityForRule(original,maps){return maps.byOriginal.get(`Regras|${norm(original)}`)||fallbackRuleRow(original)}

function hrefFor(row){
  if(!row)return '#';
  if(row.modulo==='Regras'){
    if(row.id)return `regras.html?ref=${encodeURIComponent(row.id)}#ref-${encodeURIComponent(row.id)}`;
    return `regras.html#regra-${slug(row.original||row.nome)}`;
  }
  const base=row.url||ROUTES[row.modulo]||'referencia.html';
  if(row.id&&base!=='referencia.html')return `${base}?ref=${encodeURIComponent(row.id)}#ref-${encodeURIComponent(row.id)}`;
  if(row.id)return `referencia.html?id=${encodeURIComponent(row.id)}`;
  return base;
}
...
function parseCurated(spec,maps){
  ...
  if(value.startsWith('#')){
    const id=value.slice(1),indexed=maps.byId.get(id);
    if(indexed?.modulo===module)return indexed;
    return {modulo:module,id,original:id,nome:SPECIAL_LABELS[s]||module,url:ROUTES[module]||'referencia.html'};
  }
  return maps.byOriginal.get(`${module}|${norm(value)}`)||{modulo:module,id:null,original:value,nome:value,url:ROUTES[module]||'referencia.html'};
}
...
function renderRefs(entry,maps,allOriginals){
  const off=seeAlso(entry,allOriginals);
  const officialLinks=off.refs.map(original=>{
    const row=entityForRule(original,maps),label=ptRule(original)?.nome||original;
    return `<a href="${esc(hrefFor(row))}">${esc(label)}</a>`;
  }).join(' · ');
...
function renderOfficial(entry,maps,allOriginals){
  const rule=ptRule(entry.original),entity=maps.byOriginal.get(`Regras|${norm(entry.original)}`)||null;
...
function focusRequested(){
  ...
  if(id){
    ...
    if(el){if(el.tagName==='DETAILS')el.open=true;el.classList.add('hub-ref-target');setTimeout(()=>el.scrollIntoView({behavior:'smooth',block:'center'}),80);return}
  }
  if(location.hash){
    const el=document.querySelector(location.hash);
    if(el){if(el.tagName==='DETAILS')el.open=true;setTimeout(()=>el.scrollIntoView({behavior:'smooth',block:'center'}),40)}
  }
}
...
async function load(){
  ...
  const [partTexts,hub,extra,curated,markdown,index]=await Promise.all([
    ...
    fetchRequired(`dados/referencias-regras-curadas.json?v=${VERSION}`),
    fetchSrd(),
