...
export function sharedCampaignRows(storage=globalThis.localStorage){
 ...
 return Object.values(cache).filter(row=>{const membership=row?.membership,campaignId=text(membership?.campaignId||row?.payload?.campaign?.id),current=allowed.get(campaignId);return Boolean(current&&username(membership?.username||membership?.displayName)===session.username&&membership?.active!==false&&membership?.role==='player'&&row?.payload?.campaign)}).map(row=>{const campaign=row.payload.campaign,membership=allowed.get(text(row.membership.campaignId));const fallbackIds=arr(campaign.members).filter(m=>m?.active!==false).map(m=>text(m?.characterId)).filter(Boolean);return{campaign,membership,adventures:arr(row.payload.revealedAdventures),characterIds:[...new Set([...arr(row.characterIds).map(text).filter(Boolean),...fallbackIds,text(membership?.characterId)].filter(Boolean))],readOnly:true}})
}
...
export function canOpenCharacter(characterId,storage=globalThis.localStorage){const session=readCollaborationSession(storage);if(!session||session.isMaster)return true;return visibleCharacterIds(storage).includes(text(characterId))}
export function canEditCharacter(characterId,storage=globalThis.localStorage){const session=readCollaborationSession(storage);if(!session||session.isMaster)return true;return assignedCharacterIds(storage).includes(text(characterId))}
